...
const downloadMediaByResponce = (responseBy: string, response: { data: { data: object[] } }, dirname: string, userName: string | undefined): Promise<void> => {
  ...
  // メディアツイートのみを抽出
  let mediatweets = response.data.data.filter((tweetData: object) => hasMedia(tweetData))


  // ツイートすべてを取得していた場合、RTのみを取得
  if (responseBy === "tweet") {
    mediatweets = mediatweets.filter((tweetData: object) => tweetData.retweeted)
      .map((tweetData: object) => tweetData.retweeted)
  }
